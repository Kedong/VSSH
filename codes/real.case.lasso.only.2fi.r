setwd('D:/Kfiles/Dropbox/VSSH')

library(glmnet)
library(ncvreg)

source('codes/vanilla_lasso.wo.quad.r')
...
#gene_x.3 = gene_x.1^2
gene_x = as.matrix(cbind(gene_x.1,gene_x.2))
...
coef.mat.lasso.poly = matrix(,nrow=sim.num,ncol=q*(q+1)/2)
info.lasso.poly = numeric(sim.num)
coef.mat.lassoori.poly = matrix(,nrow=sim.num,ncol=q*(q+1)/2)
...
for (sim in 1:sim.num) {
	...
	for (t in 1:9) {
		fit.lasso.poly = si(x=x1.tr, y=y.tr, s=0.1*t, beta_ori=T)
