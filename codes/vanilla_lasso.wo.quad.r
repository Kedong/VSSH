## variable selection with interaction
## formulation: OLS + lambda(s*main + (1-s)*interaction)
## the input is main effects only. the code creates all two-way interactions by itself.
## update on 12/20/2015

si = function(x, y, s, lambda=NULL, beta_ori=FALSE) {
	# user can specify lambda, otherwise glmnet default.
	# user can choose if original betas are returned.
	
	p = dim(x)[2]

	muy = mean(y)
	newy = y-muy

	x1 = scale(x)
	mux = as.vector(attributes(x1)$'scaled:center')
	sdx = as.vector(attributes(x1)$'scaled:scale')

	## create interaction term
	x2 = t(apply(x1, 1, combn, 2, prod))
	
	## take care of two tuning parameters for main and interaction effects
	x1 = x1/s
	x2 = x2/(1-s)

	newx = cbind(x1,x2)

	## fit lasso
	fit = glmnet(newx, newy, lambda=lambda, standardize=FALSE, intercept=TRUE)
	ahat = as.vector(fit$a0)
	bhat = as.matrix(fit$beta)
	bhat[1:p,] = bhat[1:p,]/s
	bhat[-(1:p),] = bhat[-(1:p),]/(1-s)

	## scale intercept and beta back to original
	if (beta_ori==TRUE) {
		bhat_ori = bhat
		ahat_ori = ahat
		bhat_ori[-(1:p),] = apply(bhat[-(1:p),],2,"/",(combn(sdx,2,prod)))

		bhat_int = matrix(,p,ncol(bhat))
		for (i in 1:p){
			xmulti = rep(1,p)
			xmulti[i] = 0
			mux[i] = 1
			a = combn(mux,2,prod) * (rep(1,p*(p-1)/2) - combn(xmulti,2,prod))
			bhat_int[i,] = colSums(apply(bhat_ori[-(1:p),],2,"*",a))
			mux = as.vector(attributes(x1)$'scaled:center')
		}
		bhat_ori[1:p,] = apply(bhat[1:p,],2,"/",sdx) - bhat_int
		ahat_ori = ahat - colSums(apply(bhat[1:p,],2,"*",mux/sdx)) +
				colSums(apply(bhat_ori[-(1:p),],2,"*",combn(mux,2,prod))) + muy

		return(list(ahat=ahat, bhat=bhat, ahat_ori = ahat_ori, bhat_ori = bhat_ori, mux=mux, sdx=sdx, muy=muy, lambda=fit$lambda))
	} else {
		return(list(ahat=ahat, bhat=bhat, mux=mux, sdx=sdx, muy=muy, lambda=fit$lambda))
	}
}

