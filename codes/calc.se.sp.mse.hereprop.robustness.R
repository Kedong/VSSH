setwd('D:/KC_file/Dropbox/VSSH/results/cond1.robust')

# for proportion of violation #
source(paste(dirname(dirname(getwd())),"/codes/calc.prop.heredity.r",sep=""))
...
coef = list(coef.mat.lasso.poly,coef.mat.lassoori.poly)
for (i in 1:2) {
if (sum(beta1)!=1){
...
se[i,] = (1*(abs(coef[[i]][,1:q][,which(beta1!=0)]) > eps[1])+
	apply(abs(coef[[i]][,(q+1):(q*(q+1)/2)][,which(beta2!=0)]) > eps[2],1,sum)+
	apply(abs(coef[[i]][,(q*(q+1)/2+1):(q*(q+3)/2)][,which(beta3!=0)]) > eps[3],1,sum))/sum(beta!=0)
sp[i,] = (apply(abs(coef[[i]][,1:q][,which(beta1==0)]) <= eps[1],1,sum)+
	apply(abs(coef[[i]][,(q+1):(q*(q+1)/2)][,which(beta2==0)]) <= eps[2],1,sum)+
...
rownames(se) = c("se.vanilla_lasso", "se.plain_lasso")
rownames(sp) = c("sp.vanilla_lasso", "sp.plain_lasso")
...
colnames(mse.table) = c("vanilla_lasso", "plain_lasso")
