...
for (sim in 1:sim.num) {
	...
	newx1.te = scale(x1.te, fit.ourstep$mux, fit.ourstep$sdx)
	newx2.te = t(apply(newx1.te, 1, combn, 2, prod))
	newx.te = cbind(newx1.te, newx2.te)
	newy.te = as.vector(y.te - fit.ourstep$muy)
	yhat.te.ourstep = fit.ourstep$ahat + newx.te%*%fit.ourstep$bhat
	info.ourstep[sim,] = mean((yhat.te.ourstep-newy.te)^2)
...
final.info = cbind(info.our,info.lasso,info.step,info.shim)
